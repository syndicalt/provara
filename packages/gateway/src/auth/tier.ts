...
/**
 * Non-middleware tier check for Team+ features (#210 audit logs is the
 * first such feature — basic audit visibility is table-stakes for Team;
 * longer retention / SIEM export remain Enterprise-tier differentiators).
 *
 * Returns true for Team, Enterprise, Self-Hosted Enterprise, operator
 * tenants, and self-host deployments. Free and Pro get false.
 */
export async function tenantHasTeamAccess(
  db: Db,
  ...
  return sub.tier === "team" || sub.tier === "enterprise" || sub.tier === "selfhost_enterprise";
}
