...
export async function reseedDemoTenant(db: Db, now: Date = new Date()): Promise<void> {
  ...
  const contextEvents = [
    {
      ...
      savedTokens: 4100,
      reductionPct: 32.85,
      avgRelevanceScore: 0.68,
      lowRelevanceChunks: 2,
      rerankedChunks: 6,
      avgFreshnessScore: 0.74,
      staleChunks: 2,
      conflictChunks: 2,
      conflictGroups: 1,
      compressedChunks: 5,
      compressionSavedTokens: 980,
      compressionRatePct: 10.47,
      nearDuplicateChunks: 2,
      conflictSourceIds: ["refunds-current.md#2", "refunds-legacy.md#8"],
      conflictDetails: [
        {
          id: "conflict-1",
          kind: "numeric",
          chunkIds: ["refunds-current.md#2", "refunds-legacy.md#8"],
          sourceIds: ["refunds-current.md#2", "refunds-legacy.md#8"],
          topicTokens: ["refund", "paid", "window"],
          leftValue: "30 days",
          rightValue: "14 days",
          score: 0.76,
          severity: "medium",
      ...
      duplicateSourceIds: ["refunds-copy.md#1", "billing-faq#10", "help-center#23"],
      nearDuplicateSourceIds: ["refunds.md#4", "billing-faq#9"],
      riskScanned: true,
      flaggedChunks: 0,
      quarantinedChunks: 1,
    ...
    },
    {
      ...
      rerankedChunks: 4,
      avgFreshnessScore: 0.81,
      staleChunks: 1,
      conflictChunks: 0,
      conflictGroups: 0,
      compressedChunks: 3,
      compressionSavedTokens: 640,
      compressionRatePct: 8.35,
      nearDuplicateChunks: 1,
      conflictSourceIds: [],
      conflictDetails: [],
      duplicateSourceIds: ["security-policy#8", "sso-runbook#6"],
      nearDuplicateSourceIds: ["security-policy#2"],
      riskScanned: true,
      flaggedChunks: 1,
      quarantinedChunks: 0,
    ...
    },
    {
      ...
      conflictGroups: 2,
      compressedChunks: 7,
      compressionSavedTokens: 1210,
      compressionRatePct: 10.17,
      nearDuplicateChunks: 3,
      conflictSourceIds: ["agent-runbook#5", "agent-runbook#12", "tool-calls#4"],
      conflictDetails: [
        {
          id: "conflict-1",
          kind: "status",
          chunkIds: ["agent-runbook#5", "agent-runbook#12"],
          sourceIds: ["agent-runbook#5", "agent-runbook#12"],
          topicTokens: ["agent", "runbook", "tool"],
          leftValue: "enabled",
          rightValue: "disabled",
          score: 0.84,
          severity: "medium",
        },
        {
          id: "conflict-2",
          kind: "numeric",
          chunkIds: ["agent-runbook#12", "tool-calls#4"],
          sourceIds: ["agent-runbook#12", "tool-calls#4"],
          topicTokens: ["tool", "handoff", "timeout"],
          leftValue: "2 hours",
          rightValue: "24 hours",
          score: 0.91,
          severity: "high",
      ...
      duplicateSourceIds: ["agent-runbook#12", "tool-calls#7", "handoff#3"],
      nearDuplicateSourceIds: ["agent-runbook#11", "tool-calls#4", "handoff#2"],
      riskScanned: true,
      flaggedChunks: 0,
      quarantinedChunks: 0,
      riskySourceIds: [],
    ...
    },
    {
      ...
      savedTokens: 520,
      reductionPct: 9.96,
      avgRelevanceScore: 0.49,
      lowRelevanceChunks: 2,
      rerankedChunks: 1,
      avgFreshnessScore: 0.55,
      staleChunks: 2,
      conflictChunks: 0,
      conflictGroups: 0,
      compressedChunks: 1,
      compressionSavedTokens: 180,
      compressionRatePct: 3.69,
  ...
  for (const event of contextEvents) {
    await db.insert(contextOptimizationEvents).values({
      ...
      nearDuplicateChunks: event.nearDuplicateChunks,
      inputTokens: event.inputTokens,
      outputTokens: event.outputTokens,
      savedTokens: event.savedTokens,
      reductionPct: event.reductionPct,
      avgRelevanceScore: event.avgRelevanceScore,
      lowRelevanceChunks: event.lowRelevanceChunks,
      rerankedChunks: event.rerankedChunks,
      avgFreshnessScore: event.avgFreshnessScore,
      staleChunks: event.staleChunks,
      conflictChunks: event.conflictChunks,
      conflictGroups: event.conflictGroups,
      compressedChunks: event.compressedChunks,
      compressionSavedTokens: event.compressionSavedTokens,
      compressionRatePct: event.compressionRatePct,
      conflictSourceIds: JSON.stringify(event.conflictSourceIds),
      conflictDetails: JSON.stringify(event.conflictDetails),
      duplicateSourceIds: JSON.stringify(event.duplicateSourceIds),
      nearDuplicateSourceIds: JSON.stringify(event.nearDuplicateSourceIds),
  ...
  for (const event of contextEvents) {
    ...
    const unusedSourceIds = [...event.duplicateSourceIds, ...event.nearDuplicateSourceIds, ...event.riskySourceIds];
    await db.insert(contextRetrievalEvents).values({
      ...
      nearDuplicateRatePct: Number(((event.nearDuplicateChunks / event.inputChunks) * 100).toFixed(2)),
      riskyRatePct: Number(((riskyChunks / event.inputChunks) * 100).toFixed(2)),
      conflictRatePct: Number(((event.conflictChunks / event.inputChunks) * 100).toFixed(2)),
