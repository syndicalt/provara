...
function TestCard({ test, onUpdate }: { test: AbTest; onUpdate: () => void }) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
      <div
        className={`flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-zinc-800/30 ${test.autoGenerated ? "border-l-2 border-indigo-500/70" : ""}`}
        onClick={() => setExpanded(!expanded)}
      >
        <div className="flex items-center gap-3">
          ...
          {test.autoGenerated && (
            <span
              title={test.sourceReason || "Auto-generated from a tied routing cell"}
              className="px-2 py-0.5 rounded text-xs font-medium bg-indigo-900/50 text-indigo-300"
            >
              Auto
            </span>
          )}
          ...
          {test.resolvedWinner && (
            <span className="text-xs text-emerald-400">→ {test.resolvedWinner}</span>
          )}
        </div>
